//
// Created by travi on 2021-11-09.
//
/*
...
void testJumble() {
    ...
    //There are 2 test cases that must be tested separately. Since the test cases exit the program once thrown only one can be tested at a time.
    /*
    //Test exception for wrong difficulty entry
    cout << "Testing Difficulty Exception" << endl;
    JumblePuzzle* jpt1;
    try {
        jpt1 = new JumblePuzzle("hello", "Easy");
    } catch (BadJumbleException& e) {
        cerr << e.what();
        return;
    }

    //Test exception for wrong word length
    cout << "Testing Word Exception" << endl;
    JumblePuzzle* jpt2;
    try {
        jpt2 = new JumblePuzzle("helloooooooooo", "easy");
    } catch (BadJumbleException& e) {
        cerr << e.what();
        return;
    }
    */
    //Test puzzle formation size
    cout << "Testing Size Formation" << endl;
    JumblePuzzle jpt3("see", "easy");
    cout << "This should display a grid of 6x6." << endl;
    showJumble(jpt3.getJumble(), jpt3.getSize());

    JumblePuzzle jpt4("see", "medium");
    cout << "This should display a grid of 9x9." << endl;
    showJumble(jpt4.getJumble(), jpt4.getSize());

    JumblePuzzle jpt5("see", "hard");
    cout << "This should display a grid of 12x12." << endl;
    showJumble(jpt5.getJumble(), jpt5.getSize());


    //Test for int getSize()
    cout << "Testing accessor for size" << endl;
    JumblePuzzle jpt6("see", "easy");
    cout << "This should be a size of 6." << endl;
    cout << "Size: "<<jpt6.getSize()<<endl;

    //Test for int getRowPos() getColPos() and getDirection
    cout << "Testing accessor for row, column and direction" << endl;
    JumblePuzzle jpt7("fast", "easy");
    cout << "The word fast should be located at : Row: "<<jpt7.getRowPos()<<" Col: "<<jpt7.getColPos()<<" Direction: "<<jpt7.getDirection()<<endl;
    showJumble(jpt7.getJumble(), jpt7.getSize());

    //Test getJumble()
    cout << "Testing accessor Jumble" << endl;
