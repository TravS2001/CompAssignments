...
def buildHull(points):
    ...
    if len(points)==2: #handle base case with two points
        #set clockwise and counter-clockwise points to eachother
        points[0].cwPoint = points[1]
        points[0].ccwPoint = points[1]
        points[1].cwPoint = points[0]
        points[1].ccwPoint = points[0]
    if  len(points)==3: #handle base case for three points
        x=turn(points[0], points[1], points[2]) #use the turn() function to determine what kind of turn the three points make
        if x==1: #left turn
            #set clockwise and counter-clockwise points
            points[0].ccwPoint = points[1]
        ...
        if x==2: #right turn
            ...
            points[1].ccwPoint = points[0]
            points[0].cwPoint = points[1]
    if len(points)>3:                           #if there are more than 3 point then use divide and conquer method to call build hull on half of the points
        leftSide=points[:len(points)//2]        #set left side to first half of points (array has already been sorted in order of ascending x)
        rightSide=points[(len(points)//2):]     #set right side to other half of points
        buildHull(leftSide)                     #recursively call buildHull with half of points
        buildHull(rightSide)
        indx = len(leftSide) - 1                #get indx of rightmost point in leftside

        l1=leftSide[indx]                       #make copies of left and right starting points
        r1=rightSide[0]
        l2=leftSide[indx]
        r2=rightSide[0]

        while turn(l1.ccwPoint, l1, r1) == 1 or turn(l1, r1, r1.cwPoint) == 1: #walk up hull
            if turn(l1.ccwPoint, l1, r1) == 1:  #if condition 1 is met
                delPoint=l1                     #make copy of current point before moving up
                l1 = l1.ccwPoint                #move up
                delPoint.ccwPoint=None          #delete pointer between previous and current points
            else:                               #if condition 2 is met
                delPoint=r1                     #make copy of current point before moving up
                r1 = r1.cwPoint                 #move up
                delPoint.cwPoint=None           #delete pointer between previous and current points

        while turn(l2.cwPoint, l2, r2) == 2 or turn(l2, r2, r2.ccwPoint) == 2: #walk down hull
            if turn(l2.cwPoint, l2, r2) == 2:   #if condition 1 is met
                delPoint=l2                     #make copy of current point before moving down
                l2 = l2.cwPoint                 #move down
                delPoint.cwPoint=None           #delete pointer between previous and current points
            else:                               #if condition 2 is met
                delPoint=r2                     #make copy of current point before moving down
                r2 = r2.ccwPoint                #move down
                delPoint.ccwPoint=None          #delete pointer between previous and current points

        l1.cwPoint=r1       # set new left and right pointers for top and bottom of hull
        r1.ccwPoint=l1
        l2.ccwPoint = r2
        r2.cwPoint = l2
